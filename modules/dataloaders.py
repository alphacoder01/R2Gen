...
from .datasets import MultiImageDataset, SingleImageDataset
...
class R2DataLoader(DataLoader):
    def __init__(self, args, tokenizer, split, shuffle):
        ...
        if split == 'train':
            if self.args.use_aug:
                self.transform = A.Compose([
                            ...
                            A.augmentations.crops.transforms.RandomCrop(224,224, always_apply=True),
                            ToTensorV2()])

            elif self.args.use_xrayvision:
                self.transform = A.Compose([
                            A.RandomBrightnessContrast(p=0.3),
                            A.CLAHE(p=0.3),
                            A.ColorJitter(p=0.3),
        ...
        if self.dataset_name == 'multi-image':
            self.dataset = MultiImageDataset(self.args, self.tokenizer, self.split, transform=self.transform)
        else:
            self.dataset = SingleImageDataset(self.args, self.tokenizer, self.split, transform=self.transform)
